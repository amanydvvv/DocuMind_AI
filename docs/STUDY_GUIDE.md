...
*   **Model Selection for Free Tier Limits:** Google Gemini free tier enforces strict daily caps (e.g. 20 requests/day). Groq production models like `llama-3.1-8b-instant` (pinned default) or `llama-3.3-70b-versatile` provide higher operational limits (15 RPM / 1,500 RPD), making them ideal for high-throughput testing and development.
...
- [x] Supabase Connection Pooler Ports (Direct 5432 vs Session/Transaction Pooler 6543)
...
#### Q: "How did you debug a cross-origin login failure that only happened in production?"
> *"A login POST produced a 308-405 chain because Vercel's `VITE_API_URL` env var was set to a full Markdown link (`[https://...](https://...)`): the leading `[` made `fetch` treat it as a relative path and resolve against the frontend's own origin. The key insight is that `import.meta.env.VITE_*` values are **baked into the bundle at build time**, never read at runtime — and platform env vars override any committed `.env` file, so the repo was always clean. Since curl testing hit the backend directly, the poison only surfaced by extracting the actual baked value from the **deployed minified bundle** (`assets/index-*.js`), the definitive source for what Vite compiled in. The `redirect_slashes=False` backend hardening removed the redirect class for all clients, but the cure was setting the env var to a bare absolute URL and redeploying — env changes never apply retroactively."*
