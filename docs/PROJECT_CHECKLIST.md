...
- [x] **Startup Lifecycle (as-built)**: `backend/app/main.py` lifespan hook only runs `await init_db()` on startup and `await close_db()` on shutdown (`main.py:30-36`). NOTE: The previously planned `_validate_generative_model()` startup guard (Google ListModels + generateContent smoke-test) does NOT exist in the current codebase — model availability depends on choosing a stable pinned model (`GENERATIVE_MODEL=llama-3.1-8b-instant`) and the Groq/Gemini fallback cascade in `generation.py` instead.
...
- [x] Plumbing tests (`backend/tests/test_eval_harness.py`, `test_eval_cli.py`) + baseline archival (`backend/results/baseline.json`, 37 entries: 100.0% marker recall / 90.9% retrieval / 97.0% groundedness / 81.8% completeness; 0 negative-control violations)
