...
|---|---|---|---|
| **Document Ingestion & Gemini OCR** | Upload files, hash duplicate detection, chunk text, batch embed, and store in pgvector. Uses **Gemini Multimodal Vision (`gemini-3.5-flash`)** for scanned image-based PDFs when text vector layer is empty. | ✅ Done | `app/routers/documents.py`, `app/services/ingestion.py` |
...
| **LLM & OCR Engine** | LangChain + Gemini Flash | Seamlessly chains text/vision prompts to Google's embeddings (`gemini-embedding-001`) and generation/OCR APIs (`gemini-3.5-flash`). |
