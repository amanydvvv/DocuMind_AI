...
- **Document Ingestion & Gemini Multimodal OCR**: Upload PDF or Markdown files with text vector extraction. Automatic **Gemini Flash Vision (`gemini-3.5-flash`) OCR fallback** transcribes text from scanned image-based PDFs (e.g. certificates and receipts) when text vector layer returns 0 characters.
...
| **LLM, Embeddings & OCR** | [Google Gemini](https://ai.google.dev/) + [LangChain](https://www.langchain.com/) | `gemini-embedding-001` (768d), `gemini-3.5-flash` text generation & vision OCR |
