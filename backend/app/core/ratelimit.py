...
def _rate_limit_key(request: Request) -> str:
    ...
    2. request.client.host: Socket connection IP fallback when CF-Connecting-IP
       ...
       X-Forwarded-For is ignored when CF-Connecting-IP is absent to prevent
       client-supplied header spoofing attacks on direct-origin routes.
    """
    cf_ip = request.headers.get("cf-connecting-ip")
    forwarded = request.headers.get("x-forwarded-for")
    peer = request.client.host if request.client else "none"
    logger.info("RATE_LIMIT_KEY_DEBUG cf_connecting_ip=%r x_forwarded_for=%r peer=%r", cf_ip, forwarded, peer)
